...
![Core deps](https://img.shields.io/badge/core_dependencies-zero-blue)
...
- 🚀 **单 EXE** —— 核心 CLI/Web 纯 Go 标准库零依赖；托盘功能用 `fyne.io/systray`（Windows，build-tag 隔离，不影响其它平台编译）
...
- 📌 **系统托盘常驻** —— `tray` 后台驻留，图标颜色随告警等级变化，超阈值弹 Windows toast，点开即仪表盘
- 📦 **多命令 CLI** —— list / top / detail / project / export csv / watch / serve / messages / tray
...
| `tray [addr]` | **系统托盘常驻** + 后台 Web 仪表盘（Windows；端口被占自动避让） |
| `help` | 帮助 |
...
后台常驻 Windows 系统托盘，无需开浏览器也能盯住 Context（仅 Windows）。底层用 `fyne.io/systray`，通过 build-tag 隔离，非 Windows 平台编译为 stub（不影响其它命令）。

```bash
ClaudeContext.exe tray              # 托盘常驻 + 后台 Web 仪表盘（127.0.0.1:8765）
ClaudeContext.exe tray 9999         # 自定义端口
```

**功能：**
- **图标颜色随最高告警等级变化**（绿/黄/橙/红/爆红），鼠标悬停 tooltip 显示 `66% · 2 活跃` 或 `⚠ 1 会话 ≥95%`
- **超阈值弹 Windows 原生 toast**（活跃会话首次跨 95%，best-effort 调 PowerShell WinRT toast；失败则仅图标转红）
- 右键菜单：**打开仪表盘** / **立即刷新** / **退出**；单击图标打开仪表盘
- 后台复用 Web 仪表盘（端口自动避让），每 5 秒刷新一次
- `Ctrl+C` 或退出菜单结束
...
├── tray/
│   ├── tray.go              托盘状态计算（共享）
│   ├── tray_windows.go      systray 集成 + ICO 生成 + toast（Windows）
...
└── ui/
    ...
    └── table.go             对齐表格
cmd/                         list/top/detail/project/export/watch/serve/messages/tray 命令实现
docs/                        设计文档（需求稿）
```

## 路线图

**已完成**：真实 Token 计数 · 智能估算回退 · 项目名同名消歧 · 会话开始时间 · 活跃会话识别 · 预计填满时间 · 全局 Top 消息 · Web 仪表盘 + REST API · 搜索/过滤/排序 · 标签页状态图标 · 端口自动避让 · **Windows 系统托盘常驻 + toast 报警** · CSV 导出 · 颜色报警 + 浏览器通知

**待办**：历史趋势图 / Session 增长曲线 · 每日 Token 增长统计 · 官方 Anthropic tokenizer 接入（Exact 模式）
